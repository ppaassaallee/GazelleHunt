...
function renderTeam() {
  ...
  return `<div class="stack">${pageIntro('Platform control', 'Users & companies', 'Platform super administrators can approve accounts, assign company access, and resend secure password setup links.', `<span class="badge badge-${pending.length ? 'orange' : 'teal'}">${pending.length} pending</span>`)}<section class="card"><div class="card-header"><div><h3>Approval queue</h3><p>Confirm company and minimum required role before activation. Approval sends a sign-in email automatically.</p></div></div>${pending.length ? `<div class="approval-list">${pending.map((user) => `<article class="approval-row"><div class="person"><div class="person-avatar">${initials(user.name)}</div><div><strong>${esc(user.name)}</strong><span>${esc(user.email)} · requested ${esc(user.requested_company_name || 'no company')}</span></div></div><select class="select" id="approve-role-${user.id}"><option value="recruiter">Recruiter</option><option value="admin">Company admin</option></select><select class="select" id="approve-company-${user.id}"><option value="">Create/use company below</option>${companyOptions}</select><input class="input" id="approve-company-name-${user.id}" value="${esc(user.requested_company_name || '')}" placeholder="Company name"><div class="approval-actions"><button class="button button-primary" data-approve-user="${user.id}">Approve</button><button class="button button-secondary" data-reject-user="${user.id}">Reject</button></div></article>`).join('')}</div>` : '<div class="empty-panel"><h3>No accounts awaiting approval</h3><p>New public registrations will appear here.</p></div>'}</section><section class="card"><div class="card-header"><div><h3>Platform accounts</h3><p>Update role, company, status, or resend a secure password setup link.</p></div><span class="badge badge-neutral">${managed.length} accounts</span></div><div class="table-scroll"><table class="account-table"><thead><tr><th>User</th><th>Company</th><th>Role</th><th>Status</th><th>Last sign-in</th><th>Actions</th></tr></thead><tbody>${managed.map((user) => { const protectedSuperAdmin = user.role === 'super_admin'; const accessLinkButton = `<button class="button button-secondary" data-reset-user="${user.id}" ${user.status === 'active' ? '' : 'disabled'}>${icon('key')}Send access link</button>`; return `<tr><td><strong>${esc(user.name)}</strong><br><span class="empty-value">${esc(user.email)}</span></td><td>${protectedSuperAdmin ? esc(user.company_name || 'Platform') : `<select class="select account-select" id="manage-company-${user.id}">${state.companies.map((company) => `<option value="${company.id}" ${company.id === user.company_id ? 'selected' : ''}>${esc(company.name)}</option>`).join('')}</select>`}</td><td>${protectedSuperAdmin ? 'super admin' : `<select class="select account-select" id="manage-role-${user.id}"><option value="recruiter" ${user.role === 'recruiter' ? 'selected' : ''}>Recruiter</option><option value="admin" ${user.role === 'admin' ? 'selected' : ''}>Company admin</option></select>`}</td><td>${protectedSuperAdmin ? statusBadge(user.status) : `<select class="select account-select" id="manage-status-${user.id}"><option value="active" ${user.status === 'active' ? 'selected' : ''}>Active</option><option value="suspended" ${user.status === 'suspended' ? 'selected' : ''}>Suspended</option><option value="rejected" ${user.status === 'rejected' ? 'selected' : ''}>Rejected</option></select>`}</td><td>${formatDate(user.last_login_at)}</td><td><div class="account-actions">${protectedSuperAdmin ? '<span class="empty-value">Protected super admin</span>' : `<button class="button button-secondary" data-save-user="${user.id}">${icon('check')}Save</button>`}${accessLinkButton}</div></td></tr>`; }).join('')}</tbody></table></div></section></div>`;
}
...
function bulkResendEligible(candidate, testId) {
  return Boolean(testId && !candidate.assessment_id && Number(candidate.attempts_remaining || 0) > 0);
}
...
function renderReports() {
  ...
  const generateLabel = aiActive
    ...
    : aiNeedsUpgrade
      ...
      : (state.reportLocale === 'es' ? 'Generar análisis' : 'Generate analysis');
...
function scoreBand(value, es) {
  const score = Number(value);
  if (!Number.isFinite(score)) return { key: 'missing', label: es ? 'Sin dato' : 'No data', tone: 'neutral', explanation: es ? 'No hay evidencia suficiente para interpretar esta dimensión.' : 'There is not enough evidence to interpret this dimension.' };
  if (score >= 80) return { key: 'strong', label: es ? 'Sólido' : 'Strong', tone: 'teal', explanation: es ? 'La evidencia es consistente y favorable; confirme con ejemplos concretos.' : 'Evidence is consistent and favorable; confirm with concrete examples.' };
  if (score >= 65) return { key: 'aligned', label: es ? 'Adecuado' : 'Aligned', tone: 'blue', explanation: es ? 'Hay ajuste suficiente, con algunos puntos normales por confirmar.' : 'There is sufficient alignment, with normal points to confirm.' };
  if (score >= 50) return { key: 'mixed', label: es ? 'Por confirmar' : 'Confirm', tone: 'orange', explanation: es ? 'La evidencia es mixta; úselo como foco de entrevista y seguimiento.' : 'Evidence is mixed; use it as an interview and follow-up focus.' };
  return { key: 'low', label: es ? 'Bajo' : 'Low', tone: 'red', explanation: es ? 'Hay tensión relevante con las condiciones del puesto; valide antes de avanzar.' : 'There is a relevant tension with role conditions; validate before moving forward.' };
}

function overallThresholds(es) {
  const rows = es
    ? [['80-100', 'Potencial observado sólido', 'Priorizar confirmación de ejemplos y condiciones de éxito.'], ['65-79', 'Potencial adecuado', 'Avanzar con preguntas focalizadas sobre lo que podría afectar permanencia.'], ['50-64', 'Potencial condicionado', 'Confirmar condiciones, expectativas y apoyo antes de decidir próximos pasos.'], ['0-49', 'Evidencia limitada', 'Revisar ajuste al puesto y considerar evidencia adicional.']]
    : [['80-100', 'Strong observed potential', 'Prioritize example confirmation and success conditions.'], ['65-79', 'Adequate potential', 'Move forward with focused questions about what may affect tenure.'], ['50-64', 'Conditional potential', 'Confirm conditions, expectations, and support before next steps.'], ['0-49', 'Limited evidence', 'Review role fit and consider additional evidence.']];
  return `<div class="threshold-table">${rows.map(([range, label, action]) => `<div><strong>${range}</strong><span>${label}</span><small>${action}</small></div>`).join('')}</div>`;
}

function dimensionInsight(label, value, es) {
  const band = scoreBand(value, es);
  return `<article class="dimension-insight"><div><span>${esc(label)}</span><strong>${value == null ? '—' : Number(value).toFixed(1)}</strong></div><b class="tone-${band.tone}">${esc(band.label)}</b><p>${esc(band.explanation)}</p></article>`;
}
...
function recruiterText(value, es) {
  return aiAssessment.recruiterText(value, es ? 'es' : 'en');
}

function confidenceLabel(value, es) {
  const labels = es
    ? { low: 'Baja', moderate: 'Moderada', high: 'Alta' }
    : { low: 'Low', moderate: 'Moderate', high: 'High' };
  return labels[value] || recruiterText(value, es);
}

function scenarioSignalLabel(value, es) {
  const labels = es
...
function scenarioConstructLabel(value, es) {
  const labels = es
    ? { role_reality: 'Adaptación a las condiciones del puesto', work_reliability: 'Respuesta ante dificultades', stay_intention: 'Decisión de permanencia' }
    : { role_reality: 'Adjustment to role conditions', work_reliability: 'Response to work challenges', stay_intention: 'Decision to remain' };
  return labels[value] || recruiterText(value, es);
}

function analysisParagraphLabels(es) {
  return es
    ? ['Resultado general', 'Ajuste al puesto e interés de permanencia', 'Respuesta ante situaciones laborales', 'Acciones recomendadas para supervisión', 'Qué confirmar en entrevista']
...
function renderReport(report) {
  ...
  const paragraphLabels = analysisParagraphLabels(copy.es);
  const readable = (value) => recruiterText(value, copy.es);
  ...
  const dimensionInsights = `<section class="report-section threshold-panel"><div class="section-title compact"><div><h3>${copy.es ? 'Umbrales de lectura' : 'Interpretation thresholds'}</h3><p>${copy.es ? 'Los rangos ayudan a priorizar qué confirmar; no son una decisión automática.' : 'Ranges help prioritize what to confirm; they are not an automatic decision.'}</p></div></div>${overallThresholds(copy.es)}<div class="dimension-insights">${dimensionInsight(copy.fit, report.subscales.fit.score, copy.es)}${dimensionInsight(copy.intent, report.subscales.intent.score, copy.es)}${dimensionInsight(copy.reliability, report.subscales.reliability.score, copy.es)}${dimensionInsight(copy.context, report.subscales.context.score, copy.es)}</div></section>`;
  ...
  const scenarioSection = `<section class="report-section"><div class="section-title compact"><div><h3>${copy.es ? 'Cómo respondió a tres situaciones laborales' : 'How the candidate handled three work situations'}</h3><p>${copy.es ? 'Muestra la respuesta original y qué conducta concreta aporta como evidencia.' : 'Shows the original answer and the concrete behavior it provides as evidence.'}</p></div><span class="badge badge-neutral">${scenarios.length} / 3</span></div><div class="scenario-evidence">${scenarios.length ? scenarios.map((entry, index) => { const finding = findings.find((item) => item.scenario_id === (entry.scenario_id || entry.id)); return `<article><span>${index + 1}</span><div><strong>${esc(copy.es ? entry.question_es : entry.question_en)}</strong><p>${esc(entry.response_text)}</p>${finding ? `<div class="scenario-finding"><b>${esc(scenarioSignalLabel(finding.signal, copy.es))}</b><span>${esc(readable(copy.es ? finding.finding_es : finding.finding_en))}</span></div>` : ''}<small>${esc(scenarioConstructLabel(entry.construct, copy.es))} · ${formatDuration(entry.response_ms)}</small></div></article>`; }).join('') : `<p>${copy.es ? 'Sin respuestas de escenarios en este resultado.' : 'No scenario responses are available for this result.'}</p>`}</div></section>`;
  const retryLabel = state.busy ? (copy.es ? 'Generando…' : 'Generating…') : (copy.es ? 'Generar o reintentar' : 'Generate or retry');
  const aiSection = `<section class="report-section ai-report"><div class="section-title compact"><div><p class="eyebrow">${copy.es ? 'Lectura asistida para reclutamiento' : 'Recruiter-ready assisted interpretation'}</p><h3>${analysis?.title ? esc(readable(analysis.title)) : (copy.es ? 'Análisis integrado del candidato' : 'Integrated candidate analysis')}</h3><p>${copy.es ? `Generado con ${esc(aiProvider)} a partir del cuestionario y las tres situaciones laborales.` : `Generated with ${esc(aiProvider)} from the questionnaire and all three work situations.`}</p></div><span class="badge badge-${analysis ? 'teal' : 'orange'}">${esc(analysis ? (copy.es ? 'Completo' : 'Complete') : visibleStatus.replaceAll('_', ' '))}</span></div>${analysis?.paragraphs?.length === 5 ? `<p class="executive-summary">${esc(readable(analysis.executive_summary || ''))}</p><div class="analysis-paragraphs">${analysis.paragraphs.map((paragraph, index) => `<article><span>${index + 1}</span><div><h4>${esc(paragraphLabels[index])}</h4><p>${esc(readable(paragraph))}</p></div></article>`).join('')}</div><div class="action-columns"><div><h4>${copy.es ? 'Preguntas listas para la entrevista' : 'Interview questions ready to ask'}</h4><ul>${readableList(analysis.interview_focus).map((item) => `<li>${esc(item)}</li>`).join('')}</ul></div><div><h4>${copy.es ? 'Acciones para capacitación y supervisión' : 'Training and management actions'}</h4><ul>${readableList(analysis.support_actions).map((item) => `<li>${esc(item)}</li>`).join('')}</ul></div></div><div class="ai-provenance"><code>${esc(aiProvider)}</code><code>${esc(report.aiAnalysis.model || '')}</code><code>${esc(report.aiAnalysis.prompt_version || '')}</code></div>` : `<div class="empty-analysis"><p>${esc(statusCopy[visibleStatus] || visibleStatus)}</p>${canRetryAi ? `<button class="button button-secondary" data-action="generate-ai" ${state.busy ? 'disabled' : ''}>${icon('refresh')}${retryLabel}</button>` : `<button class="button button-secondary" data-action="refresh-ai" ${state.busy ? 'disabled' : ''}>${icon('refresh')}${copy.es ? 'Actualizar estado' : 'Refresh status'}</button>`}</div>`}</section>`;
  return `<article class="report-document"><header class="report-cover"><div><p class="eyebrow">Gazelle Assessment</p><h2>${copy.es ? 'Reporte de Potencial de Permanencia' : 'Tenure Potential Report'}</h2><p>${esc(report.name)} · ${esc(report.role)}${report.site ? ` · ${esc(report.site)}` : ''}</p></div><div class="report-date"><span>${copy.es ? 'Completado' : 'Completed'}</span><strong>${formatDate(report.completedAt, copy.es ? 'es' : 'en')}</strong></div></header><div class="report-shell"><aside class="report-profile"><div class="score-ring" style="--score-angle:${report.potentialIndex / 100 * 360}deg"><div><strong>${report.potentialIndex.toFixed(1)}</strong><span>/ 100</span></div></div><span class="score-caption">${copy.es ? 'Índice de Potencial de Permanencia' : 'Tenure Potential Index'}</span><strong class="report-band">${copy.band}</strong><div class="profile-meta"><div><span>${copy.es ? 'Ajuste al puesto' : 'Role alignment'}</span><strong>${alignment ? `${alignment.rating} / 5` : '—'}</strong></div><div><span>${copy.es ? 'Calidad' : 'Quality'}</span><strong>${esc(qualityLabel(report.quality.status, copy.es))}</strong></div></div><p class="interpretive-note">${copy.es ? 'El índice resume el cuestionario. La lectura de 1 a 5 combina el cuestionario con las tres situaciones laborales y se explica por separado.' : 'The index summarizes the questionnaire. The 1–5 interpretation combines it with the three work situations and is explained separately.'}</p></aside><div class="report-main"><section class="report-section score-profile"><div class="section-title compact"><div><h3>${copy.es ? 'Resultados del cuestionario' : 'Questionnaire results'}</h3><p>${copy.es ? 'Cada dimensión se puntúa de 0 a 100. Las tres primeras tienen el mismo peso.' : 'Each dimension is scored from 0 to 100. The first three have equal weight.'}</p></div><span class="badge badge-${qualityTone}">${esc(qualityLabel(report.quality.status, copy.es))}</span></div>${dimensionBar(copy.fit, report.subscales.fit.score)}${dimensionBar(copy.intent, report.subscales.intent.score)}${dimensionBar(copy.reliability, report.subscales.reliability.score)}${dimensionBar(copy.context, report.subscales.context.score, true)}</section>${dimensionInsights}${alignmentSection}${aiSection}${scenarioSection}<section class="report-section report-guidance"><div><h4>${copy.es ? 'Apoyos que pueden favorecer la permanencia' : 'Support that may help retention'}</h4><ul>${supports.length ? supports.map((label) => `<li>${esc(label)}</li>`).join('') : `<li>${copy.es ? 'No disponibles' : 'Not available'}</li>`}</ul></div><div><h4>${copy.es ? 'Cómo utilizar este resultado' : 'How to use this result'}</h4><p>${copy.es ? 'Combine este perfil con una entrevista estructurada y otra evidencia relacionada con el puesto. No lo utilice como una decisión automática.' : 'Combine this profile with a structured interview and other job-related evidence. Do not use it as an automatic decision.'}</p></div></section></div></div></article>`;
}
...
function renderSettings() {
  ...
  const recipientStatus = diagnostics?.recipient ? `<span class="badge badge-${diagnostics.recipient.blocked ? 'red' : 'teal'}">Test recipient ${diagnostics.recipient.blocked ? 'blocked' : 'not blocklisted'}</span>` : '';
  const recipientDetail = diagnostics?.recipient ? `${Number(diagnostics.recipient.transactionalEmailCount || 0)} transactional messages found for recipient${diagnostics.recipient.reason ? ` · ${esc(diagnostics.recipient.reason)}` : ''}` : '';
  const diagnosticsHtml = diagnostics ? `<div class="email-diagnostics"><div><strong>Latest Brevo activity</strong><span>${diagnostics.provider.latestAccountEventAt ? formatDate(diagnostics.provider.latestAccountEventAt) : 'No provider events found in the last two days'}</span><span>API account: ${esc(diagnostics.account?.email || 'unknown')}</span></div><div class="diagnostic-counts">${senderStatus}${recipientStatus}</div><div class="diagnostic-counts">${diagnosticPlans || '<span class="badge badge-orange">No active email credits reported</span>'}</div><div class="diagnostic-counts">${diagnosticCounts || '<span class="badge badge-orange">No events</span>'}</div><div class="diagnostic-sample"><strong>Latest test message</strong><span>${Number(diagnostics.provider.sampleMessageLookupCount || 0) || diagnostics.recipient?.latestMessageMatchedByRecipient ? 'Found in Brevo' : 'Not found in Brevo activity'}${sampleCounts ? ` · ${esc(sampleCounts)}` : ' · no delivery event'}</span><span>${recipientDetail}</span></div>${diagnosticEvents.length ? `<div class="diagnostic-events">${diagnosticEvents.slice(0, 6).map((event) => `<span><b>${esc(event.event)}</b>${esc(event.date || 'No timestamp')}${event.tag ? ` · ${esc(event.tag)}` : ''}${event.matchedInvitation ? ' · matched' : ''}</span>`).join('')}</div>` : ''}</div>` : '';
...
function renderRunner() {
  ...
  if (runner.stage === 'intro') {
    const conditions = runner.roleConditions?.[locale] || (es ? ['Horario de trabajo informado y expectativas de asistencia', 'Conversaciones consecutivas con clientes', 'Metas de calidad y productividad'] : ['Published work schedule and attendance expectations', 'Back-to-back customer conversations', 'Quality and productivity targets']);
...
async function refreshAiAnalysis() {
  ...
  render();
  try {
    await fetchJson(`/api/assessments/${encodeURIComponent(report.id)}/ai-analysis`, { method: 'PATCH', body: '{}' });
    await loadWorkspace({ silent: true });
  } catch (error) {
    toast(error.message);
  } finally {
...
async function sendUserPasswordReset(userId) {
  ...
  try {
    ...
    toast('Secure password setup link sent through Brevo.');
  } catch (error) { toast(error.message); }
  finally { state.busy = false; render(); }
}
...
function bindEvents() {
  ...
  document.querySelectorAll('[data-action]').forEach((button) => button.addEventListener('click', () => {
    ...
    if (action === 'refresh-ai') refreshAiAnalysis();
