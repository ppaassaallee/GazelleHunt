...
assert.match(server, /reconcileStaleDeliveryStates/);
assert.match(server, /45 \* 1000/);
assert.match(server, /gazelle_invitation_delivery_failed/);
assert.match(server, /candidates: await listCandidates\(env, user\)/);
...
assert.match(app, /Send \/ resend/);
assert.match(app, /Select all eligible visible candidates/);
assert.match(app, /Last send failed/);
assert.match(app, /error\.candidates/);
