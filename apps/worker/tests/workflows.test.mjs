...
const [appSource, indexSource, legacyServerSource, readmeSource, buildSource, auditSource, messagingSource, journeysSource, templatesSource, portalSource] = await Promise.all([
...
assert.match(journeyHtml, /Generic API webhooks/);
assert.match(journeyHtml, /API webhook/);
assert.match(journeyHtml, /data-add-journey-step/);
assert.match(journeyHtml, /data-move-journey-step/);
...
assert.match(serverSource, /journey_api_url_invalid/);
assert.match(serverSource, /sendJourneyApiEvent/);
assert.match(serverSource, /api_webhook_rejected/);
...
assert.match(serverSource, /advanceAiAnalysis/);
assert.match(appSource, /refreshAiAnalysis/);
assert.match(appSource, /Interpretation thresholds|Umbrales de lectura/);
